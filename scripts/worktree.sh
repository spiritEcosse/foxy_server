...
copy_env_files() {
    ...
    if [[ -f ".env" ]]; then
        print_info "Copying .env to worktree..."
        ...
        if command -v direnv &>/dev/null; then
            print_info "Running direnv allow in worktree..."
            (cd "$worktree_path" && direnv allow)
            print_success "direnv allow done — env vars will auto-load on cd"
        else
            print_warning "direnv not installed — run 'direnv allow' manually after cd"
        fi
    else
        print_warning ".env not found - skipping environment file copy"
    fi
}
...
show_usage() {
    ...
    echo ""
    echo "Options:"
    echo "  --no-env    Skip copying .env file to the new worktree (for add)"
    echo "  --force     Force removal of worktree even if it has modified/untracked files (for remove)"
    echo ""
    echo "Examples:"
    echo "  $0 add feature-login                    # Creates worktree in ../feature-login with .env"
    echo "  $0 add feature-login /tmp/feature-login # Creates worktree in /tmp/feature-login with .env"
    echo "  $0 add feature-login --no-env           # Creates worktree without copying .env"
